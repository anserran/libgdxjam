...
public class Collider implements Component, Poolable {
	...
	public boolean intersects(Collider collider) {
		if (this.circle && collider.circle) {
			...
			rect2.set(collider.pos.x, collider.pos.y, collider.size.x,
					collider.size.y);
			return rect1.overlaps(rect2);
		} else {
			return this.circle ? intersectCircleRectangle(this, collider)
					: intersectCircleRectangle(collider, this);
		}
	}
	...
	private boolean intersectCircleRectangle(Collider circle, Collider rectangle) {
		...
		return rect1.set(rectangle.pos.x, rectangle.pos.y, rectangle.size.x,
				rectangle.size.y).contains(circle.pos)
				|| Intersector.intersectSegmentCircle(p1.set(left, bottom),
						p2.set(right, bottom), circle.pos, squareRadius)
				|| Intersector.intersectSegmentCircle(p2, p1.set(right, top),
						circle.pos, squareRadius)
				|| Intersector.intersectSegmentCircle(p1, p2.set(left, top),
						circle.pos, squareRadius)
				|| Intersector.intersectSegmentCircle(p2, p1.set(left, bottom),
						circle.pos, squareRadius);
	}
