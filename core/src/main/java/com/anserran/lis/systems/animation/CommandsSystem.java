        ...
        if (!velocity.has(entity) && !interpolate.has(entity)) {
            ...
            switch (command) {
                case THRUST:
                    ...
                    interpolate.set(Velocity.class, v.x, v.y, C.THRUST_TIME, C.THRUST_INTERPOLATION);
                    entity.add(interpolate);
                    break;
                case ROTATE_CW:
                case ROTATE_CCW:
                    interpolate = engine.createComponent(Interpolate.class);
                    interpolate.set(Rotation.class, command == Command.ROTATE_CCW ? 90 : -90, C.THRUST_TIME, C.THRUST_INTERPOLATION);
                    entity.add(interpolate);
                    break;
